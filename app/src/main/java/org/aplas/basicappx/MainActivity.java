...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        unitType.setOnCheckedChangeListener(
                new RadioGroup.OnCheckedChangeListener() {
                    ...
                    public void onCheckedChanged(RadioGroup group, int checkedId) {
                        ...
                        ArrayAdapter<CharSequence> itemValuesAdapter;
                        if (selected.getText().equals("Temperature")) {
                            itemValuesAdapter = ArrayAdapter.createFromResource(unitType.getContext(),
                                    R.array.tempList, android.R.layout.simple_spinner_item);
                            imgView.setImageResource(R.drawable.temperature);
                            itemValuesAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
                            unitOri.setAdapter(itemValuesAdapter);
                            unitConv.setAdapter(itemValuesAdapter);
                            inputTxt.setText("0");
                            outputTxt.setText("0");
        ...
        formBox.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                ImageView formulaImg = findViewById(R.id.imgFormula);

                formulaImg.setVisibility((isChecked) ? View.VISIBLE : View.INVISIBLE);
            }
        });
    ...
    protected String strResult(double val, boolean rounded) {
        DecimalFormat f = new DecimalFormat((rounded) ? "#.##" : "#.#####");
        return String.valueOf(f.format(val));
    }

    void doConvert() {
        RadioButton selectedRadio = (RadioButton) findViewById(unitType.getCheckedRadioButtonId());
        double inpText = Double.parseDouble(inputTxt.getText().toString());
        double convert = convertUnit(selectedRadio.getText().toString(), unitOri.getSelectedItem().toString(), unitConv.getSelectedItem().toString(), inpText);
